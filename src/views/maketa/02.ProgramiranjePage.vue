...
export default {
  ...
  setup() {
    ...
    const fetchData = async () => {
      ...
      if (Array.isArray(record.objekti_programiranje)) {
        record.objekti_programiranje.forEach((value) => {
    ...
    const confirmChanges = async () => {
      ...
      const objekti_programiranje = newModalSections.map(section => section.done); // The rest are objekti_programiranje

      const { error } = await supabase
        .from('Projekti')
        .update({ objekti_programiranje })
        .eq('ime_projekta', ime_projekta);
    ...
    return { add, chartRef, isOpenRef, setOpen, data, modalSections, updateStatus, 
      confirmChanges, selectedProject, close, checkmark, };
