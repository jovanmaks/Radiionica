...
export default {
  ...
  setup() {
    ...
    const fetchData = async () => {
      ...
      if (Array.isArray(record.objekti_crtanje)) {
        record.objekti_crtanje.forEach((value) => {
    ...
    const confirmChanges = async () => {
      ...
      const objekti_crtanje = newModalSections.map(section => section.done); // The rest are objekti_crtanje

      const { error } = await supabase
        .from('Projekti')
        .update({ objekti_crtanje })
        .eq('ime_projekta', ime_projekta);
