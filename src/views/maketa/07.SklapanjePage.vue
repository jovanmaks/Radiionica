...
<template>
  <base-layout page-title="Склапање" page-default-back-link="/tabs">
...
import { print, qrCode, checkmarkDone, cloudUpload, share, add, checkmark, close } from "ionicons/icons";
...
export default {
  ...
  setup() {
    ...
    const fetchData = async () => {
      ...
      if (Array.isArray(record.objekti_sklapanje)) {
        record.objekti_sklapanje.forEach((value) => {
    ...
    const confirmChanges = async () => {
      ...
      const objekti_sklapanje = newModalSections.map(section => section.done); // The rest are objekti_sklapanje

      const { error } = await supabase
        .from('Projekti')
        .update({ objekti_sklapanje })
        .eq('ime_projekta', ime_projekta);
    ...
    return { add, chartRef, isOpenRef, setOpen, data, modalSections, updateStatus, confirmChanges, selectedProject, close, checkmark, };
