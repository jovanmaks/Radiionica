  ...
  setup() {
    ...
    const fetchData = async () => {
      ...
      const { data, error } = await supabase
        .from('Projekti')
        .select('objekti_pripremaFarbanje')
      ...
      if (Array.isArray(record.objekti_pripremaFarbanje)) {
        record.objekti_pripremaFarbanje.forEach((value) => {
    ...
    const confirmChanges = async () => {
      ...
      const objekti_pripremaFarbanje = newModalSections.map(section => section.done); // The rest are objekti_pripremaFarbanje

      const { error } = await supabase
        .from('Projekti')
        .update({ objekti_pripremaFarbanje })
        .eq('ime_projekta', ime_projekta);
    ...
    return { add, chartRef, isOpenRef, setOpen, data, modalSections, close, checkmark,
       updateStatus, confirmChanges, selectedProject };
